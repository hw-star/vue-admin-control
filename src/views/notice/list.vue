        ...
        <el-table-column label="是否显示以及操作" align="center">
          <template slot-scope="scope">
            <el-switch
              ...
              :inactive-value="0"
              active-color="#13ce66"
              @change="noticeShow(scope.row.id, scope.row.noActive)"
            >
            </el-switch>
            <router-link :to="'/notice/details/' + scope.row.id">
...
export default {
  ...
  methods: {
    getlist(page = 1) {
      this.page = page;
      noticeApi
        .getNoticeListPage(this.page, this.limit, this.fuzzyquery)
        .then((response) => {
    ...
    handleDelete(id) {
      this.$confirm("此操作将删除该通知公告, 是否继续?", "提示", {
        ...
        .then(() => {
          noticeApi.deleteNoticeId(id).then((response) => {
            this.$message({
              type: "success",
              message: "删除成功!",
            });
    ...
    noticeShow(id, stateCode) {
      noticeApi.ShowNotice(id, stateCode).then((response) => {
        this.$message({
          type: "success",
          message: "操作成功!",
        }).catch((error) => {});
    ...
    moreDeleteData() {
      this.$nextTick(() => {
        if (this.checkList.length == 0) {
          ...
          this.$confirm("此操作将删除通知公告, 是否继续?", "提示", {
            confirmButtonText: "确定",
            cancelButtonText: "取消",
            type: "warning",
          })
            .then(() => {
              noticeApi.moreDeleteNotice(this.checkList).then((response) => {
                this.$message({
                  message: "操作成功",
                  type: "success",
                });
